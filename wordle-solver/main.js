...
function findWordOnDay(){
    const startDate = new Date("6/19/2021");  
    let chosenDate = new Date(month.value + "/" + day.value + "/" + year.value);
    console.log(month.value + "/" + day.value + "/" + year.value);
    let daysUntil = Math.floor((chosenDate.getTime() - startDate.getTime())/(1000*60*60*24));
    if (isNaN(daysUntil)){
        alert("Invalid Date")
    }else if( daysUntil >= words.length || daysUntil < 0){
        alert("Date out of bounds, try between 19/06/2021 and 20/10/2027")
    }else{
        document.getElementById("wordAlt").innerHTML = words[daysUntil];
    }
}
